...
page is permitted. Two schemes to handle the CW bit are permitted:

WARNING: #TODO: Is there a relation between the CW and the W/X bit? Should cap
writes be rejected if the X bis is set?#

* If <<pcc>> grants store capability permission (see 2.1.2. Architectural Permissions),
and a capability is written (or an AMO instruction is executed) to a virtual page with the CW bit clear, a store page fault exception is raised.
Note that if <<pcc>> does not grant store capability permission, this would have
cause an access fault (or whatever default error behavior)already.
...
clear. No fault exception is raised.
...
WARNING: #TODO: Clarify last note to be more clear.#
...
strongly encouraged, but not required, to support CD.

When all of the following condition apply
* a capability store or AMO instruction is executed
* the tag bit of the capability being written is set
* the <<pcc>> grants store capability permission
* the address written corresponds to a virtual page with the CD bit clear.
* the CW bit of that page is set
then two schemes to manage the CD bit are permitted:

* a store page fault exception is raised. The CD bit remains unchagned.
* the implementation sets the corresponding bit in the PTE. The PTE update must
be atomic with respect to other accesses to the PTE, and must atomically check
